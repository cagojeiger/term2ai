...
#### PTY 래퍼 코어
- **목적**: 기본적인 의사 터미널 작업
- **구성요소**: 프로세스 생성, 기본 I/O, 리소스 관리
- **의존성**: ptyprocess, pexpect
...
### 입력 흐름
```
사용자 입력 → UI 계층 → 플러그인 필터 → 세션 기록 → 
ANSI 처리 → 터미널 상태 → PTY 코어 → 셸 프로세스
```

### 출력 흐름
```
셸 프로세스 → PTY 코어 → 버퍼 관리 → ANSI 파싱 → 
필터 시스템 → 세션 기록 → 플러그인 처리 → UI 표시
```
